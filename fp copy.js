...
let numorderindex
let manIndex
let ManifestOpen = document.querySelector("body > table:nth-child(16) > tbody > tr > td").innerText
...
for (let i = 2; i < scans; i++) {
    ...
    if (scanDate === date) {
        ...
        if(orderArr.indexOf(order) !== -1){continue} 
            else{orderArr.push(order)}
    }
    else if(scanDate === null){continue}
    else{continue}
}
...
for (let i = 0; i < numOrders; i++) {
    ...
    if(orderArr.indexOf(orderNum) !== -1){
        ...
        let stopIndex = missedStopArr.indexOf(stop)
        missedStopArr.splice(stopIndex, 1)
...
console.log("Unloaded Stop Numbers")
console.log('--------------------------')
console.log(missedStopArr)
console.log('--------------------------')
